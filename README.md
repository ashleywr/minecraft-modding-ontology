...
is both `seed` and `compostable` and `placeable`.
...
This separation means the vocabulary of *what things are* (`facets/`) and the
organization of *how they're browsed* (`categories/`) can evolve independently, and
contributors can work on one without touching the other.
...
The path `facets/natural/seed.yml` tells you the facet ID is `natural/seed`, and it lives
conceptually in the *natural things* area of the vocabulary. You can browse the folder
tree on GitHub and understand the structure without reading any documentation.
...
describe the item and where it should go. If you want to fix it yourself:

1. Find the facet that should apply — browse `facets/` until you find the right one
...
Translation files contain only `label` and `description` — no detection rules. Open a
pull request. Missing translations always fall back to English automatically.
...
The file path is the facet's ID: `facets/natural/seed.yml` → `natural/seed`.

**`matches`** — the item has at least one of these properties  
**`requires`** — the item must also have all of these (additional constraints)  
**`except_when`** — excluded if any of these match (overrides `matches`)
...
when an item could route to multiple subcategories — `high` subcategories are preferred.
...
Facet IDs are derived from file paths: `facets/natural/seed.yml` → `natural/seed`.
...
[CC0 1.0 Universal](LICENSE) — public domain dedication. No attribution required.
